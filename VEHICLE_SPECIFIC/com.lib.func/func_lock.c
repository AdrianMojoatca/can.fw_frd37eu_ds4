...
/*==========================================================================*/
// $Id: func_lock.c 4137 2012-09-20 16:41:44Z florin.olariu $
/*==========================================================================*/
...
/*==========================================================================*/
// ADI - 09.01.2017
static	Can_Sync_Flags	lock_cmd_332(Can_Msg *can_msg);
static  Can_Sync_Flags  lock_sta_331(Can_Msg *can_msg);
/*--------------------------------------------------------------------------*/
...
void func_lock_start( void  )
{
	...
	//tester_lock();
	//ATOMIC( func_lock_done = TRUE );
	//return;

	lock_counter = 0;
	
	ATOMIC( func_lock_done = FALSE );

	// ADI - 10.01.2017
	//can_sync_loop( CAN_DEV_0 , (Can_Sync_Func)lock_sta_331 ,(Can_Sync_Arg)NULL, 0x331, 0);
	//can_sync_add ( CAN_DEV_0 , (Can_Sync_Func)lock_cmd_332 ,(Can_Sync_Arg)NULL, 0x332, 0 , 1 , 10 , 200);		 
	...
	// ADI - 12.01.2017    
    can_sync_loop( CAN_DEV_0 , (Can_Sync_Func)lock_sta_331 ,(Can_Sync_Arg)NULL, 0x331, 1);
	os_tsk_wait(1000);  
	//os_tsk_wait(500);  
...
{    
	...
	if ( !func_lock_done )
	{
		can_sync_remove( CAN_DEV_0 , (Can_Sync_Func)lock_cmd_332 ) ;	
		can_sync_add ( CAN_DEV_0 , (Can_Sync_Func)lock_cmd_332 ,(Can_Sync_Arg)NULL, 0x332, 0 , 1 , 1 , 1);		
		os_tsk_wait(700);
		lock_counter++;
	}
	if (lock_counter >=4 ) return TRUE;
	return func_lock_done ; 	
}
...
void func_lock_stop( void )
{
	...
	can_sync_remove( CAN_DEV_0 , (Can_Sync_Func)lock_cmd_332 ) ;	
	can_sync_remove( CAN_DEV_0 , (Can_Sync_Func)lock_sta_331 ) ;
	func_lock_done = TRUE;
	
	//func_can_wake_leave();	

    TRACE_FUNC("\n\r[LOCK_STOP(%s)]\n\r",func_lock_done ? "ok":"error"); 	;        
}

/*--------------------------------------------------------------------------*/

 // ADI - 09.01.2017
Can_Sync_Flags lock_cmd_332( Can_Msg *can_msg )
{   
	os_tsk_wait(50);
...
Can_Sync_Flags lock_sta_331( Can_Msg *can_msg )
{   
