...
/*==========================================================================*/
static UInt8 d2d3p_cdeq(D2d3p_Port *port);
static void d2d3p_cenq(D2d3p_Port *port,UInt8 c);
static void (*msg_handler_function)( void *);
...
* @return
*/
void d2d3p_enqmsg(D2d3p_Port *port,UInt8 *ptr,UInt8 len)
{
    CRITICAL;

    C_ENTER;

    d2d3p_cenq(port,len);  // enque number of bytes in message
    while(len--)
      d2d3p_cenq(port,*ptr++); // enque each byte
    
    d2d_switch_port_tx_irq_ctrl(port,1); // enable if not enabled
    d2d3p_uart_tx_irq(port);
...
static UInt8 d2d3p_cdeq(D2d3p_Port *port)
{
    ...
    c = port->txq[port->txnr & (D2D3P_TXFIFO_SIZE-1)];
    port->txnr++;
...
static void d2d3p_cenq(D2d3p_Port *port,UInt8 c)
{
    port->txq[port->txnw & (D2D3P_TXFIFO_SIZE-1)] = c;
    port->txnw++;
}


/**
* @brief  d2d3p uart rx irq
* @param
* @return
*/
...
//			port->rxofs = (++port->rxofs)%D2D3P_RXFIFO_SIZE;
//      port->rxbuf[port->rxofs] = c; // save byte
...
//      }
    ...
    case BUS_SLEEP: // we need to echo this byte
      port->rxbuf[1] = port->lastrx;  // write 1st byte
    
      //d2d3p_enqmsg(port, &port->lastrx,1);  // echo the byte as a 1 byte message
    
      port->rxofs = 2; // current offset for next char (if there is one)
    
      port->rxlen = 2; // TODO: increase this for multibyte command
...
//          port->rxlen++;
			switch(port->lastrx)
			{
				...
				case SET_EXTENDED_STATUS:
					port->rxlen += 3;
					break;
							
				case SET_2WAY_STATUS:
					port->rxlen += 4;
					break;
...
//      smsg_submit(port->rxmsg);
      port->rxbuf[0] = port->rxlen - 1;  // write 1st byte
		
			if((port->rxlen - 1) > 1)
	            port->rxofs = 1;  // buffer is gone



		
		  msg_handler_function(port->rxbuf);
...
//    ptr = smsg_getptr(msg);
//    
//    if(d2d3p_send_filter(port, msg))  //double check to be safe
    {
        d2d3p_enqmsg(port, msg, len);  //no need send aid of sender and proto_id
    }
