...
|                                                                           |
| The OS can use either OS-provided heap memory or C/C++ malloc/free        |
| heap memory depending on the setting of this macro.                       |
|                                                                           |
| This only affects new/delete operations.  All OS requests for dynamic     |
| memory are still allocated from the OS heap.                              |
|                                                                           |
| Set to 0 to use malloc/free heap memory.                                  |
| Set to 1 to use OS allocated heap memory.                                 |
\*-------------------------------------------------------------------------*/

#ifdef __cplusplus

#ifndef OS_PROVIDES_NEW_MEM
#define OS_PROVIDES_NEW_MEM 0
#endif
...
#define CRITICAL_TSK TskPri __tsk_critvar__ ; TskTimeout __tsk_timeslice__

#define C_ENTER_TSK                                               \
do                                                                \
  {                                                               \
     __tsk_timeslice__ = os_tsk_set_timeslice (0   ,    0) ;      \
     __tsk_critvar__   = os_tsk_set_pri       (OS_PRI_MAX) ;      \
  }                                                               \
while (0)
...
do                                                                \
  {                                                               \
    (void) os_tsk_set_pri       (    __tsk_critvar__  ) ;         \
    (void) os_tsk_set_timeslice (0 , __tsk_timeslice__) ;         \
  }                                                               \
while (0)
...
#if defined (__cplusplus) && OS_PROVIDES_NEW_MEM == 1 
...
extern void          (* os_mem_fail_func)(void * , UInt16) ;  // Called if os_mem_request fails
