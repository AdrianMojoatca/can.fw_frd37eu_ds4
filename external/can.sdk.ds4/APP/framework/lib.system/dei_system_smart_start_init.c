...
/*==========================================================================*/
// $Id: dei_system_smart_start_init.c 19287 2013-02-18 12:08:05Z florin.olariu $
/*==========================================================================*/
...
static void dei_system_smart_start_error         ( Notify_Pld_Error error_type );
static void dei_system_smart_start_mts (void);
/*==========================================================================*/
/*                            V A R I A B L E S                             */
/*==========================================================================*/
	...
	notify_register:
    {
        static const Notify_Array array[] =
        {
        	...
        	{NOTIFY_WARNING                 , (Notify_Func)dei_system_smart_start_warn_away     },
					{NOTIFY_TRIGGER                 , (Notify_Func)dei_system_smart_start_engine_stopped},
        	{NOTIFY_ERROR                   , (Notify_Func)dei_system_smart_start_error         },
        	{NOTIFY_MTS_DISABLE             , (Notify_Func)dei_system_smart_start_mts          }
        };
...
{
	if(get_gearbox_type() == GEARBOX_MANUAL && !get_rs_mts())
...
void dei_system_smart_start_error (Notify_Pld_Error error_type)
{
	if(   error_type == NOTIFY_PLD_ERROR_BRAKE
	   || error_type == NOTIFY_PLD_ERROR_HOOD 
	   || error_type == NOTIFY_PLD_ERROR_LOW_TACH
	   || error_type == NOTIFY_PLD_ERROR_OVERREV
	   || error_type == NOTIFY_PLD_ERROR_LOW_BATT
	  )
		dei_system_smart_start_engine_stopped();
}
...
#ifdef PLATFORM_933
  if(( get_ss_sensor_trigger  (1) || get_ss_sensor_trigger  (2)) &&
		  (get_ss_trigger_source() == DEI_SS_TRIGGER_ZONE_SENSOR || get_ss_trigger_source() == DEI_SS_TRIGGER_ZONE_SENSOR2))
		rf_icon_set(ICON_SHOCK   ,FALSE); //shock/tilt
#else   //CM800
	if( get_ss_sensor_trigger  (1) )rf_icon_set(ICON_SHOCK   ,FALSE);
#endif	
	if( get_ss_door_trigger    () && get_door_status()) rf_icon_set(ICON_DOOR    ,FALSE);
	if( get_ss_hood_trigger    () && get_hood_status()) rf_icon_set(ICON_HOOD    ,FALSE);
	if( get_ss_trunk_trigger   () && get_trunk_status()) rf_icon_set(ICON_TRUNK   ,FALSE);
	if( get_ss_ign_trigger     () && get_ign_status()) rf_icon_set(ICON_IGNITION,FALSE);
...
/*--------------------------------------------------------------------------*/
void dei_system_smart_start_mts (void)
{
	if(get_gearbox_type() == GEARBOX_MANUAL)
		dei_system_smart_start_engine_stopped();
}
