...
/*==========================================================================*/
// $Id: dei_system_rf_panic.c 19287 2013-02-18 12:08:05Z florin.olariu $
/*==========================================================================*/
...
Boolean as_system_rf_panic_avail;
/*==========================================================================*/
/*                 F U N C T I O N   D E F I N I T I O N S                  */
/*==========================================================================*/
static void dei_system_rf_panic_available(void);
static void dei_system_rf_panic_not_available(void);
//--------------------------------------------------------------------------//
...
{	
	...
	if( get_ss_trigger()  )
	{
		dei_system_rf_panic_not_available();
		return;
	}
	
	if( as_system_rf_panic_avail )
		as_system_rf_panic_avail = FALSE;

  if((dei_system_type & (SYS_TYPE_RS | SYS_TYPE_SS)) == 0)
  {
    dei_system_rf_panic_available();   // DO allow Panic for convenience  (like mode 1)
	  return;
  }
  
//	if(dei_system_type == SYS_TYPE_RS )
//  {
//    dei_system_rf_panic_available();   //--- panic is always available in RS only systems
//	  return;
//  }
    
  if( dei_feature_get( DEI_FEAT_PANIC_MODE )== 1 )//panic mode 1
	{
		dei_system_rf_panic_available();
	}
	else if( dei_feature_get( DEI_FEAT_PANIC_MODE )== 2 )//panic mode 2
	{	  
		if( get_ign_status() )
			dei_system_rf_panic_not_available();
		else 
	...
	else if( dei_feature_get( DEI_FEAT_PANIC_MODE )== 3 )//panic mode 3
		dei_system_rf_panic_not_available();
}
...
void as_system_rf_panic( void *remote )
{
	...
	{
		as_system_rf_panic_avail = TRUE;
		dei_system_rf_panic( NULL );
	}
}
