...
static Boolean init_ver (void)
{
  ...
  if ((nvfs_size_is_valid(nvfs_obd_can_ver, sizeof(version)) == NVFS_OK) && (nvfs_rd_s(nvfs_obd_can_ver , &version, sizeof(version)) == NVFS_OK) && (version == OBD_CAN_CUR_VER))
	{
	  #if DBG_OBD_CAN == 1
...
static Boolean init_config (void)
{
  if ((nvfs_size_is_valid(nvfs_obd_can_config, sizeof(obd_can_config_default)) == NVFS_OK) && (nvfs_rd_s (nvfs_obd_can_config , &obd_can_cfg, sizeof(obd_can_cfg))== NVFS_OK))
    {
      #if DBG_OBD_CAN == 1
...
static Boolean init_err (void)
{
  if ((nvfs_size_is_valid(nvfs_obd_can_err, sizeof(obd_can_err)) == NVFS_OK) && (nvfs_rd_s (nvfs_obd_can_err , &obd_can_err, sizeof(obd_can_err))== NVFS_OK))
    {
      #if DBG_OBD_CAN == 1
		dbg_tx_text ("\nObd Can Err success") ;
	  #endif
