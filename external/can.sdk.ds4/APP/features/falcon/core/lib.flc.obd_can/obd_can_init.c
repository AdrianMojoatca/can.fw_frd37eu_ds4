...
void obd_can_init (void)
{
  ...
  // register CAN handlers
  {
    UInt16 id ;
    
    // Requests to the ECUs - if we detect this, it means a scan tool is connected - we must enter silent mode 
    can_usr_rx_reg_slow(obd_can_vars.dev_id,(Reg_Func)obd_can_scan_tool_detected,OBD_CAN_REQ_ALL_ECUS);  
  
    for(id = OBD_CAN_REQ_FIRST_ECU ; id <= OBD_CAN_REQ_LAST_ECU ; obd_can_vars.is_ext ? id+=0x0100 : id++)
    {
        can_usr_rx_reg_slow(obd_can_vars.dev_id,(Reg_Func)obd_can_scan_tool_detected,id);        
    }				 

    // Responses from the ECUs
    for(id = OBD_CAN_RESP_FIRST_ECU ; id <= OBD_CAN_RESP_LAST_ECU ; id++)
    {
  	    can_usr_rx_reg_slow(obd_can_vars.dev_id,(Reg_Func)obd_can_rx,id);              
    }
  }
