...
void  dei_sc_init    ( void ) 
{
    ...
    if(nvfs_rd_s( nvfs_sys_type , &scpp_system_type, sizeof(scpp_system_type) ) != NVFS_OK)
			scpp_system_type = sys_type_assumed;
    ...
    // creating VRTC timer which is used for Sensor adjust mode time out
    dei_sc_sensor_adjust_mode_vrtc = vrtc_f_create( (Exec_Func)dei_sc_sensor_adjust_mode_timeout ) ; 

    per_exec_register((Per_Exec_Func)dei_sc_cmd_release_tmr_update);

#if 0   //use dei_scpp_cmd function
