...
{    
	...
	switch( dei_sc_info_get( DEI_SC_INFO_SECURITY ) )
	{
		...
		{
			dei_sc_info_set( DEI_SC_INFO_SECURITY  , DEI_SECURITY_DISARMED_RS_VALET_ON );    
		...
		{
			dei_sc_info_set( DEI_SC_INFO_SECURITY  , DEI_SECURITY_DISARMED_COMBO_VALET_ON );    
    ...
    dei_sc_info_set( DEI_SC_INFO_SENSOR2_NPC           , !!(trig&NOTIFY_PLD_TRIG_S2_NPC )) ; 
	  dei_sc_info_set( DEI_SC_INFO_SENSOR3_NPC           , !!(trig&NOTIFY_PLD_TRIG_S3_NPC )) ; 
		if(ext_sensor_trigger_configured && get_ss_sensor_npc(0))
		{
			dei_sc_info_set( DEI_SC_INFO_SENSOR3_NPC, TRUE);     //extern shock trig as ZONE 7
		}
    ...
    if( DEI_SC_PAGGING() ) 
    {
         ...
         os_tsk_wait(90);  //40+50ms
         if( ((trig & (DEI_SC_UNLOCK_NPC_EVENT | DEI_SC_UNLOCK_TRIGGER_EVENT )) && (!get_ss_valet())) )
         {
